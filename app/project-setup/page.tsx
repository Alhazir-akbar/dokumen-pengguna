...
export default function WizardPage() {
  ...
  // ⏩ Fungsi Skip Langsung ke Stories
  const handleSkip = async () => {
    ...
    try {
      const { createWorkspaceIfNeeded, createProjectIfNeeded } = useWizardStore.getState() as any;
      const wsId = await createWorkspaceIfNeeded(token);
      const projId = await createProjectIfNeeded(token);

      if (projId) {
        localStorage.setItem('active_project_id', String(projId));
        if (wsId) localStorage.setItem('active_workspace_id', String(wsId));
        resetStore();
        router.push(`/stories?project_id=${projId}`);
      } else {
        router.push('/stories');
      }
    } catch (e) {
